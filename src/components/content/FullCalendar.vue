...
import { createTimeUnitId,timeUnitIdToTime,timeUnitSplit } from '@/utils/calendar-utils'
...
export default {
  ...
  methods: {
    ...
    //将获取到的 timeUnit id 解析成需要的时间格式
    handleCalendarView(timeUnit) {
      let timeStr = timeUnit.timeUnitSplit();
      for(let i = 0 ; i < timeStr.length ; i++){

        let hostTime = timeStr[i].timeUnitIdToTime();
        let calendarApi = hostTime.view.calendar
        calendarApi.unselect() // clear date selection
        calendarApi.addEvent({
          // 此处的id就是对应timeUnit的id
          id: timeStr,
